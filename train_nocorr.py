    ...
    for epoch_idx in range(start_epoch_idx, args.n_total_epoch + 1):
        ...
        for batch_idx, mini_batch_data in enumerate(dataloader):
            ...
            left, right, gt_disp, valid_mask = (
                mini_batch_data["left"],  # Bx3xHxW uint8 cpu tensor
                mini_batch_data["right"],  # Bx3xHxW uint8 cpu tensor
                mini_batch_data["disparity"].cuda(),  # BxHxW float32 gpu
                mini_batch_data["mask"].cuda(),  # BxHxW float32 gpu (1. is valid)
            )
